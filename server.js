...
// Middleware para checar permissão por role
...
// Deletar produto (admin, gestor)
app.delete("/products/:id", authMiddleware, requireRole(["admin", "gestor"]), async (req, res) => {
  const { id } = req.params;
  try {
...
// Deletar local (admin, gestor)
app.delete("/locations/:id", authMiddleware, requireRole(["admin", "gestor"]), async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query("DELETE FROM locations WHERE id = $1", [id]);
    res.status(204).end();
  } catch (err) {
    console.error("Erro deletar local:", err);
    res.status(500).json({ error: "Erro interno" });
  }
});
